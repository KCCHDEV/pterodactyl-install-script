...
setup_named_tunnel() {
    ...
    # Setup DNS when credentials exist: API first (if token), else run_route_dns
    if [[ -f "$credentials_path" ]]; then
        if [[ -n "${CLOUDFLARE_API_TOKEN:-}${CF_API_TOKEN:-}" ]]; then
            cf_delete_dns_for_hostname "$domain" 2>/dev/null
            local tid
            tid=$(get_tunnel_id "$tunnel_name" "$credentials_path")
            if [[ -n "$tid" ]] && cf_create_cname_for_hostname "$domain" "${tid}.cfargotunnel.com" 2>/dev/null; then
                log_success "DNS route: $domain -> $tunnel_name (via API)"
            elif run_route_dns "$tunnel_name" "$domain"; then
            ...
            log_warn "DNS failed. Try manually:"
            log_info "  cloudflared tunnel route dns $tunnel_name $domain"
            log_info "  (if record exists: add --overwrite-dns at the end)"
            log_info "Or use API token (Zone:DNS Edit) and retry Switch Mode."
        fi
    fi
...
# Get tunnel UUID from tunnel list or credentials file
get_tunnel_id() {
    local tunnel_name="$1"
    local creds_path="${2:-/etc/cloudflared/${tunnel_name}.json}"
    local tid
    tid=$(cloudflared tunnel list 2>/dev/null | grep -w "$tunnel_name" | grep -oE '[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}' | head -1)
    [[ -z "$tid" && -f "$creds_path" ]] && tid=$(grep -oE '"TunnelID"[[:space:]]*:[[:space:]]*"[^"]*"|"t"[[:space:]]*:[[:space:]]*"[^"]*"' "$creds_path" 2>/dev/null | grep -oE '[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}' | head -1)
    echo "$tid"
}

# Try multiple route dns syntaxes for different cloudflared versions
run_route_dns() {
        ...
        case "$choice" in
            ...
            1)
                if [[ -f "$CLOUDFLARED_CONFIG" ]]; then
                    ...
                    if [[ -n "${CLOUDFLARE_API_TOKEN:-}${CF_API_TOKEN:-}" ]]; then
                        local tid_add
                        tid_add=$(get_tunnel_id "$tunnel_name" "$creds_path")
                        if [[ -n "$tid_add" ]] && cf_create_cname_for_hostname "$hostname" "${tid_add}.cfargotunnel.com" 2>/dev/null; then
                            log_success "DNS: $hostname (via API)"
                        elif run_route_dns "$tunnel_name" "$hostname"; then
                            log_success "DNS route: $hostname -> $tunnel_name"
                        else
                            log_warn "DNS failed. Try: cloudflared tunnel route dns $tunnel_name $hostname"
                        fi
                    elif run_route_dns "$tunnel_name" "$hostname"; then
                        log_success "DNS route: $hostname -> $tunnel_name"
                    else
            ...
            5)
                if [[ -f "$CLOUDFLARED_CONFIG" ]]; then
                    ...
                    local tid_ra
                    tid_ra=$(get_tunnel_id "$tunnel_name" "$creds_path")
                    while IFS='|' read -r h _; do
                        [[ -n "${CLOUDFLARE_API_TOKEN:-}" || -n "${CF_API_TOKEN:-}" ]] && cf_delete_dns_for_hostname "$h" 2>/dev/null
                        if [[ -n "${CLOUDFLARE_API_TOKEN:-}${CF_API_TOKEN:-}" && -n "$tid_ra" ]] && cf_create_cname_for_hostname "$h" "${tid_ra}.cfargotunnel.com" 2>/dev/null; then
                            log_success "DNS: $h (via API)"
                        elif run_route_dns "$tunnel_name" "$h"; then
                            log_success "DNS: $h -> $tunnel_name"
                        else
                            log_warn "DNS failed for $h. Try: cloudflared tunnel route dns $tunnel_name $h"
                        fi
                    done < <(parse_tunnel_ingress "$CLOUDFLARED_CONFIG")
                    echo "" >&2
                    log_info "If DNS failed: export CLOUDFLARE_API_TOKEN then retry option 5 or 6."
                fi
                ;;
            6)
                if [[ -f "$CLOUDFLARED_CONFIG" ]]; then
                    ...
                    if [[ -n "${CLOUDFLARE_API_TOKEN:-}" || -n "${CF_API_TOKEN:-}" ]]; then
                        ...
                        local tid_fix
                        tid_fix=$(get_tunnel_id "$tunnel_name" "$creds_path")
                        while IFS='|' read -r h _; do
                            cf_delete_dns_for_hostname "$h" 2>/dev/null && log_info "Deleted existing record: $h"
                            if [[ -n "$tid_fix" ]] && cf_create_cname_for_hostname "$h" "${tid_fix}.cfargotunnel.com" 2>/dev/null; then
                                log_success "DNS: $h (via API)"
                            elif run_route_dns "$tunnel_name" "$h"; then
                                log_success "DNS: $h -> $tunnel_name"
                            else
